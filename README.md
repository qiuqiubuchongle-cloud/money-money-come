...
> 多链聪明钱地址画像与筛选系统。把你收藏的聪明钱地址，整理成可解释、可评分、可分层、可复盘的个人链上研究库。

`Money Money Come` 是一个面向 meme 扫链用户、链上研究员和 Agent 工作流的公开 Skill。它基于 **OnchainOS / OKX 链上数据能力**，核心能力是把钱包地址分析成可解释的链上人格画像：这个地址像什么人、擅长什么、风险在哪里、值不值得长期跟踪，以及它应该被当成“发现源”“确认源”还是“背景噪音”。

它现在更像一层 **地址判断系统**，不是另一个原始信号搬运工。信号通知只是辅助输出，真正的核心是先把地址看懂、筛干净、分清用途。

市场上已经有很多原始信号、钱包跟踪和快速交易工具，所以 `Money Money Come` 最该补的不是“更多提醒”，而是：

- 把多条原始信号收束成更清楚的判断
- 把钱包从“单地址”变成“可解释的链上人格”
- 把地址从“备注名”变成“可复盘的跟踪决策”
...
| 原始信号工具更擅长 | Money Money Come 更擅长 |
| --- | --- |
...
导入聪明钱地址
  ...
  -> 生成 Wallet Persona 链上人格画像
  -> 计算钱包价值分与信任分
  -> 给出 Wallet Decision 跟踪决策
  -> 自动分组与筛选
  -> 生成 safeSignalPool
  -> 按你的规则监控同组集中买入
...
一句话理解：它不是帮你“看到更多买入”，而是帮你先知道“哪些地址值得看、怎么用、什么时候该降权”。
...
输入一个 BSC 或 Solana 钱包地址，Skill 会生成一份 Wallet Persona 画像和 Wallet Decision 跟踪决策：

- 身份判断：聪明钱候选、稳定盈利型、早期发现型、热点轮动型、疑似机器人、高噪音地址等
- 人设名称：早期叙事猎人、稳健盈利派、信仰加仓派、热点切换手、链上打点机、后排冲锋号等
- 置信度、真人感、机器人感
- 能力雷达：发现力、执行力、退出纪律、稳定性、低市值适配、抗噪能力、可跟随性
- 行为证据：每个人设判断都会列出链上依据
- 跟踪建议：核心观察池、观察池、降权池、排除池、休眠池
- 跟踪决策：A/B/C/D 级、信任分、用途、触发规则、失效条件
- 已实现收益
- 胜率
- 最近活跃时间
- 交易节奏
...
叙事敏感、低市值偏好、出手靠前；可同时作为发现源和确认源。同人设/同组 2 个钱包 5 分钟内买入可提醒。
```
...
Wallet Persona 会额外输出：

| 字段 | 含义 |
| --- | --- |
| `personaName` | 用户能读懂的人设名，例如“早期叙事猎人” |
| `personaIdentity` | 更客观的身份判断，例如“早期发现型聪明钱” |
| `personaConfidence` | 当前证据支撑强度 |
| `humanScore` / `botScore` | 真人交易感 / 机器人或脚本感 |
| `ability` | 发现力、执行力、退出纪律、稳定性等能力雷达 |
| `evidence` | 支撑这个判断的链上行为证据 |
| `tracking` | 应该放入哪个池子、怎么触发提醒 |

### 4. Wallet Decision 跟踪决策

光有“人设”还不够，真正有用的是下一步怎么盯。

`Wallet Decision` 会把每个地址转成四级判断：

| 等级 | 结论 | 用法 |
| --- | --- | --- |
| A | 重点跟踪 | 可进核心观察池，适合作为发现源或确认源 |
| B | 观察验证 | 进观察池，需要同组或外部热度确认 |
| C | 只看题材 | 不单独触发，只在核心地址已触发时做辅助说明 |
| D | 排除留档 | 不进信号池，主要用于减少噪音 |

它会额外输出：

| 字段 | 含义 |
| --- | --- |
| `trustScore` | 地址信任分，综合价值分、可靠度、人设置信度、可跟随性、机器人感和活跃度 |
| `watchMode` | 这个地址适合作为什么用途：发现源、确认源、加仓观察、背景参考或排除留档 |
| `reasons` | 为什么给这个结论 |
| `risks` | 当前最大风险 |
| `triggerRule` | 这个地址参与提醒时应该怎么触发 |
| `invalidators` | 什么情况说明这个地址应该降权或移出 |

这个设计的重点是把“这个地址很会买”拆成更细的使用方式：

- 早期叙事猎人：适合发现题材，但要防追高
- 稳健盈利派：适合做确认源，不一定最早
- 信仰加仓派：适合观察持续加仓，但要看退出
- 热点切换手：适合捕捉热度，不适合单点盲跟
- 链上打点机：多数情况下只用于识别噪音
...
也就是说，这个 skill 的目标不是重复做原始信号流，而是把多源信号再压一层，只留下更值得你看的那小撮。
